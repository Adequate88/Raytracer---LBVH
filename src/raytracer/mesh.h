...
class mesh {
public:
  static shared_ptr<hittable_list>
  load_obj(const std::string &filename, shared_ptr<material> default_material,
           bool smooth_shading = true, double scale = 1.0) {
    ...
    for (const auto &shape : shapes) {
      ...
      for (size_t f = 0; f < shape.mesh.num_face_vertices.size(); f++) {
        ...
        for (size_t v = 0; v < 3; v++) {
          ...
          vertices[v] =
              point3(scale * attrib.vertices[3 * idx.vertex_index + 0],
          ...
          if (idx.texcoord_index >= 0) {
            uvs[v] = vec2(attrib.texcoords[2 * idx.texcoord_index + 0],
        ...
        auto tri = make_shared<triangle>(vertices[0], vertices[1], vertices[2],
                                         normals[0], normals[1], normals[2],
                                         uvs[0], uvs[1], uvs[2], tri_mat,
  ...
  static point3
  compute_mesh_center(const shared_ptr<hittable_list> &triangles) {
    ...
    point3 center((mesh_bbox.min_x + mesh_bbox.max_x) / 2.0,
