...
## Phase 3: Wavefront Ray Tracing

The single biggest downside of the phase 1 ray tracing (referred to as megakernel ray tracing from now on) is low occupancy. Only one thread gets dispatched per sampling ray, and a lot of these threads go idle quickly. For example there could be a ray that misses all the objects in our scene, or escapes into nothing after only one bounce. The threads belonging to these rays have to idle until the very last ray finishes bouncing. To prevent this occupancy issue we decided to implement wavefront ray tracing based on the ideas of [this](https://research.nvidia.com/sites/default/files/pubs/2013-07_Megakernels-Considered-Harmful/laine2013hpg_paper.pdf) paper.

Our implementation divides the ray tracing into five compute shaders. 
 * **Generate**: This shader generates all the rays based on the position of the camera, and puts them into a ray buffer. This shader runs only once per frame. One thread processes one ray.
 * **Extend**: This shader extends all the rays in the ray buffer until they hit an object of the scene, or escape it entirelly. This shader runs in a loop before the next two shaders (shade and dispatch). One thread processes one ray.
 * **Shade**: This shader takes all the hits (or no hits) generated by the extend shader and calculates which rays "die" and which "stay alive". The rays that stay alive also contribute to the final image, the shader also calculates the throughput and radiance of these rays. The rays that stay alive bounce, this shader also generates the next rays that extend will need to process. This shader runs in a loop between extend and the next shader (dispatch). One thread processes one ray.
 * **Dispatch**: The shade shader keeps track of how many rays stay alive. This shader makes sure we only dispatch the necessary amount of threads for the next iteration of the loop. This is done by calculating the number of workgroups needed based on the number of rays still alive. This is done with the help of the [vkCmdDispatchIndirect](https://docs.vulkan.org/refpages/latest/refpages/source/vkCmdDispatchIndirect.html) instruction. This is the key step that increases occupancy compared to megakernel ray tracing. This shader runs in a loop after extend and shade. Only one thread is launched. 
 * **Finalize**: This final shader produces our final frame. When appropriate the shade shader writes into a buffer that contains the final color values of our pixels. This shader simply outputs them to the final image after our loop of extend, finalize, dispatch is finished. This shader is only ran once, and one thread processes one pixel.


### Run-time and Rendering Measurements

| Measurement | Baseline (ms) | Phase 2 (ms) | Phase 3 (ms)
| --- | --- | --- | ---
...
After switching to wavefront ray tracing we can see a marked improvement compared to the megakernel ray tracing. Despite the wavefront method having more overhead, for scenes with multiple samples (and therefore ray paths) the efficiency gained by reducing the occupancy speeds up the rendering process significanty. With the rendering times observed, real time ray tracing is feasible and implemented. 


## Phase 4: Reducing overhead and low level optimizations
